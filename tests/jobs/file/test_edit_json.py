...
from pyantz.infrastructure.config.base import PipelineConfig, JobConfig
...
_json_like_strategy = st.dictionaries(
    _json_key_strategy,
...
def test_edit_simple_json(data: dict[str, Any], key, value) -> None:
    ...
    assert edit_json_mod.nested_edit(
        data,
        key,
        value
    ) == expected
...
def test_nested_dictionary_n_level(data: dict[str, Any], keys: list[str], value: Any) -> None:
    ...
    result = edit_json_mod.nested_edit(
        data,
        '.'.join(keys),
        value
    )
