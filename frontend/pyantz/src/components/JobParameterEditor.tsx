...
export default function JobParameterEditor({
  id,
  job,
}: IJobParameterEditorProps) {
  ...
  const { simpleParameters, singleJobParameters, multiJobParameters } = useMemo(
    () =>
      jobParameterSchema
        ? splitParams(job.parameters, jobParameterSchema)
        : {
...
const NonJobParametersEditor = ({
  paramsSchema,
  parameters,
  updateJobParams,
}: IInnerParameterEditorProps) => {
  const ajv = useMemo(() => new Ajv2019(), []);
  const validate = useMemo(
    () =>
      paramsSchema && paramsSchema?.properties
        ? ajv.compile(paramsSchema)
        : () => true,
    [paramsSchema, ajv],
  ); // Re-compile only if schema changes
  ...
  return (
    ...
    >
      <JsonEditor data={parameters} setData={setData} />
    </div>
  );
};
...
const splitParams = (
  ...
  const parameterConstructor = (fieldList: string[]) => ({
    schema: {
      ...
      required: schema?.required?.filter((req_field) =>
        fieldList.includes(req_field),
      ),
    },
  ...
  return {
    simpleParameters:
      Object.keys(simpleParameters).length > 0 ? simpleParameters : null,
    singleJobParameters:
      Object.keys(singleJobParameters).length > 0 ? singleJobParameters : null,
    multiJobParameters:
      Object.keys(multiJobParameters).length > 0 ? multiJobParameters : null,
  };
};
